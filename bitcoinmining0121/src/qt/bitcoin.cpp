...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. kryptofranc_de.qm (shortcut "de" needs to be defined in kryptofranc.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. kryptofranc_de_DE.qm (shortcut "de_DE" needs to be defined in kryptofranc.qrc)
...
class kryptoFrancApplication: public QApplication
{
    Q_OBJECT
public:
    explicit kryptoFrancApplication(int &argc, char **argv);
...
kryptoFrancCore::kryptoFrancCore():
    QObject()
{
}
...
kryptoFrancApplication::kryptoFrancApplication(int &argc, char **argv):
    ...
    returnValue(0)
{
    ...
    platformName = GetArg("-uiplatform", kryptoFrancGUI::DEFAULT_UIPLATFORM);
...
{
    window = new kryptoFrancGUI(platformStyle, networkStyle, 0);
...
void kryptoFrancApplication::startThread()
{
    ...
    kryptoFrancCore *executor = new kryptoFrancCore();
...
void kryptoFrancApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        if(pwalletMain)
        {
            ...
            window->addWallet(kryptoFrancGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(kryptoFrancGUI::DEFAULT_WALLET);
...
void kryptoFrancApplication::shutdownResult(int retval)
{
...
int main(int argc, char *argv[])
{
    ...
    kryptoFrancApplication app(argc, argv);
    ...
    /// 6. Determine availability of data directory and parse kryptofranc.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        QMessageBox::critical(0, QObject::tr("KryptoFranc"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("KryptoFranc"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // kryptofranc: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("KryptoFranc didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
