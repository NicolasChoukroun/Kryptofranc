...
*Setup instructions for a Gitian build of kryptoFranc using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the kryptoFranc
...
- [Building kryptoFranc](#building-kryptofranc)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-kryptofranc-host/README.md).
...
Clone the git repositories for kryptofranc and Gitian.
...
in the kryptofranc repository under 'Fetch and build inputs' to install sources which require
...
To build kryptoFranc (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the kryptofranc repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kryptofranc/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/kryptofranc/kryptofranc
...
For example:
```bash
URL=https://github.com/laanwj/kryptofranc.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kryptofranc=${COMMIT} --url kryptofranc=${URL} ../kryptofranc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kryptofranc=${COMMIT} --url kryptofranc=${URL} ../kryptofranc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kryptofranc=${COMMIT} --url kryptofranc=${URL} ../kryptofranc/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the kryptofranc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kryptofranc/contrib/gitian-descriptors/*|sort|uniq )
...
KYFPATH=/some/root/path/kryptofranc.git
SIGPATH=/some/root/path/kryptofranc-detached-sigs.git

./bin/gbuild --url kryptofranc=${KYFPATH},signature=${SIGPATH} ../kryptofranc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kryptofranc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kryptofranc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kryptofranc-osx-build.assert
```
...
[kryptofranc/gitian.sigs](https://github.com/kryptofranc/gitian.sigs/) repository, or if that's not possible create a pull
