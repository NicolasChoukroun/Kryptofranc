KryptoFranc version 0.9.0 is now available from:

  https://kryptofranc.org/bin/0.9.0/
...
TX outputs, bloating kryptofranc's UTXO database.
...
("tell the running kryptofranc daemon to do THIS") was split into a separate
executable, 'kryptofranc-cli'. The RPC client code will eventually be removed from
kryptofrancd, but will be kept for backwards compatibility for a release or two.
...
0.1mKYF per kilobyte. During periods of heavy transaction volume, even this
...
- Add '-disablewallet' mode to let kryptofrancd run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with kryptofrancd
...
- Drop the fee required to relay a transaction to 0.01mKYF per kilobyte
...
- Added new DNS seed from kryptofrancstats.com
...
- Rebrand to `KryptoFranc`
- Move initialization/shutdown to a thread. This prevents "Not responding"
  messages during startup. Also show a window during shutdown.
- Don't regenerate autostart link on every client startup
- Show and store message of normal kryptofranc:URI
- Fix richtext detection hang issue on very old Qt versions
- OS X: Make use of the 10.8+ user notification center to display Growl-like 
  notifications
- OS X: Added NSHighResolutionCapable flag to Info.plist for better font
  rendering on Retina displays.
- OS X: Fix kryptofranc-qt startup crash when clicking dock icon
- Linux: Fix Gnome kryptofranc: URI handler
...
- Add separate kryptofranc-cli client
