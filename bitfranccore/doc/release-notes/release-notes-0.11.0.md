KryptoFranc version 0.11.0 is now available from:

  <https://kryptofranc.org/bin/kryptofranc-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer KYF/kB fee to be
...
For example, add the following to `kryptofranc.conf`:
...
related to the blockchain in the kryptofranc system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows KryptoFranc to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that KryptoFranc will 
...
For an example script see [kryptofranc-submittx](https://github.com/laanwj/kryptofranc-submittx).
...
- #5334 `cf87536` libkryptofrancconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new kryptofranc-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only kryptoFrancAmountField
- #5753 `0f44672` Add kryptofranc logo to about screen
...
- #5809 `46bfbe7` Add kryptofranc-cli man page
...
- #5849 `41259ca` contrib/init/kryptofrancd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename kryptoFranc-Qt.app to kryptoFranc-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for kryptofranc-tx)
