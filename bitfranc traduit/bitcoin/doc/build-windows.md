...
Below are some notes on how to build Bitfranc Core for Windows.

The options known to work for building Bitfranc Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Bitfranc Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Bitfranc Core applications.
...
Note that for WSL the Bitfranc Core source path MUST be somewhere in the default mount file system, for
