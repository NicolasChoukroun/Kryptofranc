KryptoFranc version 0.17.0 is now available from:

  <https://kryptofranccore.org/bin/kryptofranc-core-0.17.0/>
...
KryptoFranc is extensively tested on multiple operating systems using
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the kryptofranc.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=kryptofranc
    test.uacomment=kryptofranc-testnet
...
be used in V0.17 by starting kryptofrancd with the '-deprecatedrpc=accounts'
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in KYF. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When kryptofranc is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If kryptofranc is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart kryptofrancd with
...
- When running kryptofrancd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If kryptofrancd
...
- #11962 `0a01843` add seed.kryptofranc.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in KYF (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI kryptofranc:// (krab)
...
- #13486 `66e1a08` Move rpc/util.cpp from libkryptofranc-util to libkryptofranc-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for kryptofranc-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling kryptofranc-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to kryptofranc.conf (MarcoFalke)
...
- #12882 `d96bdd7` Make `test_kryptofranc` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.kryptofranc is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_kryptofranc` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in kryptofrancd systemd service file (Flowdalic)
- #12652 `55f490a` kryptofranc-cli: Provide a better error message when kryptofrancd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to kryptofrancd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for kryptofranc address in translations (kaplanmaxe)
...
- #13603 `453ae5e` kryptofranc-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix kryptofranc-cli --version (Empact)
...
- #12877 `2b54155` Use kryptofrancd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update kryptofranc-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust kryptofranccore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed kryptofranc-cli -help output for help2man (hebasto)
