...
KryptoFranc version *version* is now available from:

  <https://kryptofranccore.org/bin/kryptofranc-core-*version*/>
...
KryptoFranc is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer.  It is not recommended
to use KryptoFranc on unsupported systems.
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/kryptofranc/kryptofranc/blob/master/doc/zmq.md#usage)
  for details.

- The `enablebip61` option (introduced in KryptoFranc 0.17.0) is
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  (and may want to consider [contacting](https://kryptofranccore.org/en/contact/)
  the KryptoFranc project to let us know about their
...
- A new short
  [document](https://github.com/kryptofranc/kryptofranc/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/kryptofranc/kryptofranc/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/kryptofranc/kryptofranc/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/kryptofranc/kryptofranc/blob/master/doc/kryptofranc-conf.md)
  about the `kryptofranc.conf` file describes how to use it to configure
  KryptoFranc.

- A new document introduces KryptoFranc's BIP174
  [Partially-Signed kryptoFranc Transactions (PSBT)](https://github.com/kryptofranc/kryptofranc/blob/master/doc/psbt.md)
...
- The [output script descriptor](https://github.com/kryptofranc/kryptofranc/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent kryptoFranc-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  KryptoFranc to libssl vulnerabilities in the past, builders who don't
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  See the [release notes from v0.17](https://github.com/kryptofranc/kryptofranc/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- The wallet's `generate` RPC method is deprecated and will be fully
  ...
  continue using `generate` in this version, restart kryptofrancd with the
...
- The `getaddressinfo` RPC now returns an additional `solvable` boolean
  field when KryptoFranc knows enough about the address's scriptPubKey,
...
- The `importprivkey` RPC will preserve previously-set labels for
  ...
  label "cold wallet" in earlier releases of KryptoFranc, subsequently
...
- A new `kryptofranc-wallet` tool is now distributed alongside kryptoFranc
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, KryptoFranc
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to KryptoFranc's own implementation, although entropy
  gathered by KryptoFranc is fed out to OpenSSL and then read back in
  when the program needs strong randomness.  This moves KryptoFranc a
...
- On macOS, KryptoFranc now opts out of application CPU throttling
