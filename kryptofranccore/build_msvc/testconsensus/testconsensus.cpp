...
int main()
{
    std::cout << "kryptofrancconsensus version: " << kryptofrancconsensus_version() << std::endl;
    ...
    auto op0Result = kryptofrancconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, kryptofrancconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
