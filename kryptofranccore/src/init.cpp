 ...
 */
static const char* KRYPTOFRANC_PID_FILENAME = "kryptofrancd.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", KRYPTOFRANC_PID_FILENAME)));
}
 ...
 * between kryptofrancd, and kryptofranc-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", KRYPTOFRANC_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", KRYPTOFRANC_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if kryptofranc is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if kryptofranc is started while in a temporary directory.\n",
