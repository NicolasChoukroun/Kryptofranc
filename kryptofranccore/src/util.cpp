...
const char * const KRYPTOFRANC_CONF_FILENAME = "kryptofranc.conf";
const char * const KRYPTOFRANC_PID_FILENAME = "kryptofrancd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "kryptofrancd -foo=bar
...
#else
    const char* pszModule = "kryptofranc";
#endif
    if (pex)
        return strprintf(
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", KRYPTOFRANC_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure kryptoFranc Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("kryptoFranc Core") == std::string::npos) {
