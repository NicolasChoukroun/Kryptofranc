...
ClientModel is used by the main application `kryptofrancgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/kryptofranc/kryptofranc/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a kryptofranc:... link from a browser).
...
* don't change `local/kryptofranc_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "kryptofranc-qt" under "Run", which is where you can also add command line arguments)
