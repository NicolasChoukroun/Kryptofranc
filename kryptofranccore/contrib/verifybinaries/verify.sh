...
###   kryptofranccore.org and kryptofranc.org and compares them.
...
WORKINGDIR="/tmp/kryptofranc_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://kryptofranccore.org"
HOST2="https://kryptofranc.org"
BASEDIR="/bin/"
VERSIONPREFIX="kryptofranc-core-"
RCVERSIONSTRING="rc"
...
if [ $? -ne 0 ]; then
   echo "kryptofranc.org failed to provide signature file, but kryptofranccore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "kryptofranc.org and kryptofranccore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the KryptoFranc binary release signing key installed?"
   fi
